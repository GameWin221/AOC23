...
pub fn answer() -> usize {
    ...
    let seed_words: Vec<&str> = FILE_DATA
        .split_once("\r\n").unwrap().0
        .split_at(7).1
        .split_ascii_whitespace().collect();

    let seed_pairs: Vec<(usize, usize)> = seed_words
        .windows(2)
        .step_by(2)
        .map(|word| 
            (word[0].parse().unwrap(), word[1].parse().unwrap())
        ).collect();
    ...
    seed_pairs.iter().map(|(start, count)| {
        println!("{start}..{}", start+count);
        
        (*start..start+count).into_iter().map(|seed| {
