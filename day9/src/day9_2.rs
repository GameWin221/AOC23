...
fn generate_new_value(history: Vec<Vec<i32>>) -> i32 {
    ...
    for i in (0..history.len()-1).rev() {
        ...
        let right_val = history[i].first().unwrap().clone(); // 1st change compared to part 1

        new_values.push(right_val - bottom_val); // 2nd change compared to part 1, only 2 lines changed lol
    }
